...
int copy_file_linux_read_write(int infile, int outfile)
{
	...
	constexpr const uint_least32_t min_read_write_buf_size = 8u * 1024u;
	constexpr const uint_least32_t max_read_write_buf_size = 256u * 1024u;
	// Default block size, reasonable for most file systems.
    uintmax_t buf_sz = 4096u;
    ...
    while (true) {
        ssize_t sz_read = ::read(infile, buf.data(), buf_size);
        if (sz_read == 0)
            break;
        if (sz_read < 0) {
            int err = errno;
			...
			if (sz < 0) {
	            ...
	            if (offset == 0u) {
	                ...
	                if (err == EINVAL || err == EOPNOTSUPP) {
						err = copy_file_linux_read_write(infile.fd, outfile.fd);
						if (err < 0)
							goto fail;
						// Succeeded.
	                	break;
