...
namespace boost {
    namespace nowide {
        ///
        /// Instal utf8_codecvt facet into  filesystem::path such all char strings are interpreted as utf-8 strings
        ///
        inline void nowide_filesystem()
        {
            // filesystem works implicitely with utf8 on Unix systems and UTF-16 on Windows.
//            std::locale tmp = std::locale(std::locale(),new boost::nowide::utf8_codecvt<wchar_t>());
//            filesystem::path::imbue(tmp);
        }
    } // nowide
} // boost
