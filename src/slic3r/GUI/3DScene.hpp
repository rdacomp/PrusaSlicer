...
public:
    ...
    // Set Z print height range for the G-code preview. Ignored by the shells.
    virtual void        set_range(double /* low */, double /* high */) {}

    // Return an estimate of the memory consumed by this class.
    virtual size_t 		cpu_memory_used() const { 
...
public:
    ...
    std::pair<size_t, size_t>   tverts_range{ 0, size_t(-1) };
    std::pair<size_t, size_t>   qverts_range{ 0, size_t(-1) };
};

class GLVolumeComponent : public GLVolume {
public:
	GLVolumeComponent(float r = 1.f, float g = 1.f, float b = 1.f, float a = 1.f) : GLVolume(r, g, b, a) {}
    GLVolumeComponent(const float* rgba) : GLVolume(rgba) {}
    virtual ~GLVolumeComponent() = default;
...
private:
    ...
    double        		  m_sla_shift_z = 0.;
    // Bounding box of this volume, in unscaled coordinates.
    mutable BoundingBoxf3 m_transformed_bounding_box;
    // Whether or not is needed to recalculate the transformed bounding box.
    mutable bool          m_transformed_bounding_box_dirty = true;
    // Convex hull of the volume, if any.
    std::shared_ptr<const TriangleMesh> m_convex_hull;
    // Bounding box of this volume, in unscaled coordinates.
    mutable BoundingBoxf3 m_transformed_convex_hull_bounding_box;
    // Whether or not is needed to recalculate the transformed convex hull bounding box.
    mutable bool          m_transformed_convex_hull_bounding_box_dirty = true;
};

class GLVolumePrintPath : public GLVolume {
public:
    GLVolumePrintPath(const float *rgba) : GLVolume(rgba) {}
    virtual ~GLVolumePrintPath() = default;

    // Set Z print height range for the G-code preview. Ignored by the shells.
    void                set_range(double low, double high) override;
...
public:
    ...
    GLVolumePrintPath* new_toolpath_volume(const float *rgba, size_t reserve_vbo_floats = 0);
    GLVolumePrintPath* new_nontoolpath_volume(const float *rgba, size_t reserve_vbo_floats = 0);
